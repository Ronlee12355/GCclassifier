...
The `GCclassifier` package enables the molecular subtype prediction of gastric cancer through log2 scaled gene expression profiles, employing the random forest algorithm (https://www.stat.berkeley.edu/~breiman/RandomForests/). Additionally, it offers non-R users the convenience of utilizing local and online Shiny applications for streamlined GC molecular subtype prediction. The package installation requires a minimum R version of 4.1.0.
...
In this case study, we employed the `GCclassifier` package to conduct gastric cancer molecular subtype prediction based on gene expression profiles. The gene expression profiles used in this analysis belong to the ACRG cohort, derived from a microarray dataset comprising 300 gastric cancer patients known as [GSE62254](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE62254) (@Cristescu2015). Subsequently, we utilized these gene expression profiles to predict the molecular subtype using three distinct subtype systems, namely `EMP`(@Oh2018), `ACRG`(@Cristescu2015), and `TCGA`(@TCGA2014).
